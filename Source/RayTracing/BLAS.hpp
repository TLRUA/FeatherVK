...
namespace FeatherVK {
    ...
    public:
        ...
        static void release() {
            ...
            for(auto& blasBuildInfo:blasBuildInfoMap){
                if (blasBuildInfo.second != nullptr && blasBuildInfo.second->as != VK_NULL_HANDLE) {
            ...
            pendingModelIndexReferences.clear();
        }
        static void modelToBLASInput(const std::shared_ptr<Model> &model) {
            if (model == nullptr) {
                return;
            }
            const uint32_t modelIndexReference = model->getIndexReference();
            if (blasBuildInfoMap.find(modelIndexReference) != blasBuildInfoMap.end() ||
                pendingModelIndexReferences.find(modelIndexReference) != pendingModelIndexReferences.end()) {
                return;
            }
            ...
            blasInput.modelIndexReference = modelIndexReference;
            if (DiagnosticsEnabled()) {
                std::cerr << "[BLAS] enqueue model='" << model->GetName()
                          ...
                          << " vertexAddress=" << vertexBufferDeviceAddress
                          << " indexAddress=" << indexBufferDeviceAddress << "\n";
            }
            pendingModelIndexReferences.insert(modelIndexReference);
        ...
        static void buildBLAS(VkBuildAccelerationStructureFlagsKHR flags) {
            ...
            for (int i = 0; i < blasCount; i++) {
                ...
                blasBuildInfoMap[blasInputs[i].modelIndexReference] = buildInfos[i];
                ...
                const VkBuildAccelerationStructureFlagsKHR effectiveFlags =
                        (flags & ~VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_COMPACTION_BIT_KHR) |
                        (blasInputs[i].flags & ~VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_COMPACTION_BIT_KHR);
                buildInfos[i]->buildGeometryInfo.flags = effectiveFlags;
                ...
                if (DiagnosticsEnabled()) {
                    std::cerr << "[BLAS] size modelIndex=" << blasInputs[i].modelIndexReference
                              << " buildSize=" << buildInfos[i]->buildSizesInfo.accelerationStructureSize
                              << " scratchSize=" << buildInfos[i]->buildSizesInfo.buildScratchSize
                              << " flags=" << buildInfos[i]->buildGeometryInfo.flags << "\n";
                }
            ...
            for (int i = 0; i < blasCount; i++) {
                    ...
                    if (batchSize > batchSizeLimit || i == blasCount - 1) {
                        ...
                        Device::getDeviceSingleton()->endSingleTimeCommands(commandBuffer, "blas_build");
                    ...
                    if (queryPool) {
                        ...
                        Device::getDeviceSingleton()->endSingleTimeCommands(commandBuffer, "blas_compact");
                    }
    ...
    private:
        ...
        inline static std::unordered_set<uint32_t> pendingModelIndexReferences{};
        ...
        static void cmdCreateBLAS(VkCommandBuffer &commandBuffer,
            ...
            for (const auto &idx: indices) {
                ...
                const VkDeviceAddress blasAddress =
                        Device::getDeviceSingleton()->getAccelerationStructureAddressKHR(buildInfos[idx]->as);
                if (DiagnosticsEnabled()) {
                    std::cerr << "[BLAS] build modelIndex=" << blasInputs[idx].modelIndexReference
                              << " asHandle=" << buildInfos[idx]->as
                              << " asAddress=" << blasAddress
                              << " asSize=" << buildInfos[idx]->buildSizesInfo.accelerationStructureSize << "\n";
                }
