...
namespace FeatherVK {
    ...
    public:
...
#ifdef RAY_TRACING
                auto modelIt = Model::models.find(modelName);
        ...
        static void ResetTLASIdAllocator() {
            s_nextTlasId = 0;
        }
        ...
        void SetUI(std::vector<EntityDesc> *gameObjectDesc, FrameInfo &frameInfo) override {
            if (model == nullptr) {
                return;
            }
            ImGui::Text("Model:       %s", model->GetName().c_str());
            ImGui::Text("Vertices:    %u", model->getVertexCount());
            ImGui::Text("Indices:     %u", model->getIndexCount());
            ImGui::Text("Material:    %d", materialId);
            ImGui::Text("TLAS ID:     %d", tlasId);
            if (gameObjectDesc != nullptr && static_cast<size_t>(tlasId) < gameObjectDesc->size() && ImGui::TreeNode("PBR")) {
                EntityDesc &desc = gameObjectDesc->at(tlasId);
    ...
    private:
#ifdef RAY_TRACING
        static id_t AcquireTLASId() {
            return s_nextTlasId++;
        }

        inline static id_t s_nextTlasId = 0;
        id_t tlasId{};
#endif
        id_t materialId{};
