...
namespace FeatherVK {
    ...
    public:
        LightComponent() {
            name = "LightComponent";
            lightCategory = LightCategory::POINT_LIGHT;
            lightTypeStr = "Point";
            lightIndex = lightNum;
            lightNum++;
        }
...
#ifdef RAY_TRACING
        void SetUI(std::vector<EntityDesc> *, FrameInfo &frameInfo) override {
            ImGui::Text("Color:");
            ImGui::SameLine(90);
        ...
        void SetLightCategory(int category) {
            if (category == LightCategory::DIRECTIONAL_LIGHT) {
                lightCategory = LightCategory::DIRECTIONAL_LIGHT;
