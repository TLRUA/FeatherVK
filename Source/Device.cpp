...
//        std::cout << "available extensions:" << std::endl;
    ...
    void Device::endSingleTimeCommands(VkCommandBuffer &commandBuffer, const char *context) {
        ...
        if (submitResult != VK_SUCCESS) {
            std::cerr << "[Device] endSingleTimeCommands submit failed context=" << context
                      << " result=" << VkResultToString(submitResult)
                      << " (" << submitResult << ")\n";
            throw std::runtime_error("single-time command submit failed");
        }

        const VkResult waitResult = vkQueueWaitIdle(graphicsQueue_);
        if (waitResult != VK_SUCCESS) {
            std::cerr << "[Device] endSingleTimeCommands wait failed context=" << context
                      << " result=" << VkResultToString(waitResult)
                      << " (" << waitResult << ")\n";
            throw std::runtime_error("single-time command wait failed");
        }
    ...
    void Device::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
        ...
        endSingleTimeCommands(commandBuffer, "copy_buffer");
    }
    ...
    void Device::copyBufferToImage(
        ...
        vkCmdCopyBufferToImage(
                ...
                1,
                &region);
        endSingleTimeCommands(commandBuffer, "copy_buffer_to_image");
    }
    ...
    void Device::transitionImageLayout(VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout,
        ...
        endSingleTimeCommands(commandBuffer, "transition_image_layout");
    }
